  ...
  for (m = 0; m < 3; m++)
      ...
      for (i = 0; i < latsize; i++)
	{
	  ...
	  for (j = 0; j < latsize; j++)
		...
		for (b = 0; b < 3; b++)
		  {
		    rhs += cmat[(n + 3 * b) * latsize * latsize +	(j + latsize * i)] * (cos_dtkr_i * eps (0, b, m) +
						      sin_dtkr_i * eps (1, b,
									m));
		    rhs +=
							     ...
							     m) + (mf_sp[i +
								   ...
								   mf_sm[i +
									 ...
									 *
									 0]) *
		       eps (1, b, m)) * prod;
		    rhs -=                                                    //Error: all the i here contained in the s[] are actually j
		      ((s[i + latsize * 0] * deltamat[j + latsize * i] -
			0.5 * s[i + latsize * 1] * gammamat[j +
							    latsize * i]) *
		       eps (0, b,
			    m) + (s[i + latsize * 1] * deltamat[j +
								latsize * i] +
		...
		for (b = 0; b < 3; b++)
		  {
		    prod = cmat[((b + 3 * m) * latsize * latsize) +
				(j + latsize * i)];
		    rhs +=((mf_sp[j + latsize * 0] - 0.5 * mf_sm[j + latsize * 1]) * eps (0, b, n) + (mf_sp[j +
									 latsize
									 *
									 1] +
								   0.5 *
								   mf_sm[j +
									 ...
									 *
									 0]) *
		       eps (1, b, n)) * prod;
		    rhs -=                                                     //Error: all the j here contained in the s[] are actually i
		      ((s[j + latsize * 0] * deltamat[j + latsize * i] -
			0.5 * s[j + latsize * 1] * gammamat[j +
							    latsize * i]) *
		       eps (0, b,
			    n) + (s[j + latsize * 1] * deltamat[j +
								latsize * i] +
		...
		for (b = 0; b < 3; b++)
		  {
		    prod = s[i + latsize * b];
		    rhs += ((mf_cmatp[((n + 3 * 0) * latsize * latsize) +
				      (j + latsize * i)] -
			     0.5 *
			    ...
			    (mf_cmatp
			     [((n + 3 * 1) * latsize * latsize) +
			      (j + latsize * i)] -                            // <------- Error: this minus should be plus
			     0.5 *
			     mf_cmatm[((n + 3 * 0) * latsize * latsize) +
				      (j + latsize * i)]) * eps (1, b,
								 m)) * prod;
		    rhs -=                                     // this terms are useless since c[anything+(j + latsize * j)] has to be zero by definition (it is actually the last condition you impose at the end)
		      ((cmat
			[((n + 3 * 0) * latsize * latsize) +
			 (j + latsize * j)] * deltamat[j + latsize * i] -
			0.5 * cmat[((n + 3 * 1) * latsize * latsize) +
				   (j + latsize * j)] * gammamat[j +
								 latsize *
								 i]) * eps (0,
									    b,
									    m)
			...
			[((n + 3 * 1) * latsize * latsize) +
			 (j + latsize * j)] * deltamat[j + latsize * i] -
			0.5 * cmat[((n + 3 * 0) * latsize * latsize) +
				   (j + latsize * j)] * gammamat[j +
								 latsize *
								 i]) * eps (1,
									    b,
		...
		for (b = 0; b < 3; b++)
		  {
		    ...
		    rhs += ((mf_cmatp[((0 + 3 * m) * latsize * latsize) +
				      (j + latsize * i)] -
			     0.5 *
			     mf_cmatm[((1 + 3 * m) * latsize * latsize) +
				      (j + latsize * i)]) * eps (0, b,
								 n) +
			    (mf_cmatp
			     [((1 + 3 * m) * latsize * latsize) +
			      (j + latsize * i)] -                      // <------- Error: this minus should be plus
			     0.5 *
			     mf_cmatm[((0 + 3 * m) * latsize * latsize) +
				      (j + latsize * i)]) * eps (1, b,
								 n)) * prod;
		    rhs -=										// this terms are useless since c[anything+(j + latsize * j)] has to be zero by definition (it is actually the last condition you impose at the end)
		      ((cmat
			[((0 + 3 * m) * latsize * latsize) +
			 (i + latsize * i)] * deltamat[j + latsize * i] -
			0.5 * cmat[((1 + 3 * m) * latsize * latsize) +
				   (i + latsize * i)] * gammamat[j +
								 latsize *
								 i]) * eps (0,
									    b,
									    n)
			...
			[((1 + 3 * m) * latsize * latsize) +
			 (i + latsize * i)] * deltamat[j + latsize * i] -
			0.5 * cmat[((0 + 3 * m) * latsize * latsize) +
				   (i + latsize * i)] * gammamat[j +
								 latsize *
								 i]) * eps (1,
									    b,
