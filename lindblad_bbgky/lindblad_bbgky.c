  ...
  for (m = 0; m < 3; m++)
      ...
      for (i = 0; i < latsize; i++)
	{
	  ...
	  for (j = 0; j < latsize; j++)
		...
		for (b = 0; b < 3; b++)
		  {
		    rhs += cmat[(n + 3 * b) * latsize * latsize +
		...
		rhs_iter += rhs;
		//Verified Correct up to this point          
...
//Debugging only.
//    for(d_i=0;d_i<latsize;d_i++){
//       PySys_WriteStdout("\n");
//      for(d_j=0;d_j<latsize;d_j++)
//        PySys_WriteStdout(" %lf",  dcdt_mat[((d_n + 3 * d_m) * latsize * latsize) +
//                     (d_j + latsize * d_i)]);
//    }
